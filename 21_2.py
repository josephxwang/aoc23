...
def part1(lines):
    ...
    for i in range(63+1): # odd parity
        seen = set()
        next = []
        for r,c in curr:
            if (r,c) in seen:
...
# !! input has some weird peculiarities (no horizontal or vertical obstacles in line with S)
# also nothing on boundaries...
# hmm, I wonder how I can exploit those.
# also, there is a weird empty boundary around an interior diamond
...
ls = [l.strip() for l in open(f'21.in')]
p2 = part2(ls)
if p2:
    print(p2)
if not p2:
